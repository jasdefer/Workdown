...
- **Fields section.** A table in declaration order: name, type,
  ...
  *resource-backed*. `id` is the first row. Rows are not yet clickable.
  Under the type, a muted one-line summary of the type-specific
  settings (decision 4).
...
4. **Type summary line.** A pure function takes a field's `shape` and
   ...
   choice, `0 to 100` for a numeric, `at most 40h` for a duration,
   ...
   types on its own; it prints what the payload carries.
...
Tests per the web-app rule: unit tests for the pure helpers (badges,
default formatting, type summary, description cut) and the new
`pageLabel` case. No component test. No server change.
