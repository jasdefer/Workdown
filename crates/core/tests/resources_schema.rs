...
fn guard() -> SchemaGuard {
    SchemaGuard::compile("resources.schema.json", SCHEMA_JSON)
}
...
fn default_resources_yaml_validates() {
    ...
    guard().assert_valid(DEFAULT_RESOURCES_YAML);
}

#[test]
fn full_example_validates() {
    guard().assert_valid(FULL_EXAMPLE_YAML);
}

#[test]
fn empty_document_validates() {
    guard().assert_valid("# comments only\n");
}
...
fn entry_missing_id_is_rejected() {
    guard().assert_invalid("people:\n  - name: No Id\n");
}

#[test]
fn section_with_scalar_value_is_rejected() {
    guard().assert_invalid("people: just a string\n");
}
...
fn constant_missing_value_is_rejected() {
    guard().assert_invalid("constants:\n  rate:\n    type: float\n");
}
