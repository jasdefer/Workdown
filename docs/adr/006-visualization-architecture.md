...
*Note (2026-09-07):* the rule stands as written, and the web UI now offers the explicit action itself. With `serve.git_controls` on, the git pill can pull, push, and — behind a confirmation dialog that lists the files and the message — stage, commit and push. Three limits keep this inside the rule: nothing is staged or committed until the user confirms; the commit covers only the paths `config.yaml` names plus `config.yaml` itself, never the rest of the repository; and the CLI stays commit-free, so `workdown add` and `workdown set` still only touch the working tree. This is also the one exception to "UI is a shell around the CLI" above: the git surface calls the user's own `git`, and its mutating half (commit, pull, push) has no `workdown <subcommand>` twin — the terminal is that twin. Only the read-only half has one: `workdown changes` prints the same generated message the dialog proposes. The decision is recorded in `workdown-items/full-git-loop.md`.
