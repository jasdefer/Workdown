...
- **The project is loaded per request.** No cache, cold-load every time. Parsing a few hundred items takes milliseconds, well under human-perceptible latency, and it means there is no stale-cache class of bug at all. A deliberate trade, revisited only when it starts to hurt. One endpoint is exempt: `GET /api/project` answers from the boot-time `Config` in `AppState` alone, so the browser can title its tab with the project's name even while a broken schema has every other read answering 422. The cost of that exemption is the config-read-once asymmetry — an edited `config.yaml` reaches it only after a restart.
