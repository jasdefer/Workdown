...
- **Git is the user's own `git`, scoped to the workdown paths, in its own crate.** `crates/git` (`workdown-git`) is the one place git is spawned: it runs the `git` on `PATH` — credentials, hooks and signing behave as in a terminal — synchronously and under a timeout, and both the CLI (`workdown changes`, `workdown install-hooks`) and the server depend on it; neither depends on the other for git. The `/api/git` handlers (`crates/server/src/api/git.rs`) run each action on a blocking thread under one lock, so a pull and a push never overlap. Staging, committing and the pill's dirty count are limited to a scope `workdown_git::scope` builds once per process from `Config::workdown_paths` (the `paths.*` entries, `schema`, and `config.yaml` itself — the same list the pre-commit hook installer reads) as repository-relative pathspecs, so a workdown project inside a larger repository never touches its neighbours. Status still reads the whole repository once, because a pull must refuse over uncommitted work anywhere and name the files outside the scope. The commit message is generated by `crate::change_summary` in core from the parsed frontmatter of each changed item at `HEAD` and in the working tree — never from diff text; the preview that gathers those texts (`workdown_git::preview`) is also what `workdown changes` prints, so a terminal and the dialog word a change identically. Commit, pull-if-behind and push are one request with a per-step report; the dialog that confirms it is the review moment ADR-006's "no auto-commit" rule asks for, and `workdown-items/full-git-loop.md` records the decision.
