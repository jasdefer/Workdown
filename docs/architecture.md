...
- **The project is loaded per request.** No cache, cold-load every time. Parsing a few hundred items takes milliseconds, well under human-perceptible latency, and it means there is no stale-cache class of bug at all. A deliberate trade, revisited only when it starts to hurt. Two endpoints are exempt. `GET /api/project` answers from the boot-time `Config` in `AppState` alone, so the browser can title its tab with the project's name even while a broken schema has every other read answering 422; the cost of that exemption is the config-read-once asymmetry — an edited `config.yaml` reaches it only after a restart. `GET /api/schema/definition` (`workdown_core::schema_definition_data`) reads `schema.yaml` alone — hashed, parsed and re-parsed for its text blocks from the same bytes — because the schema editor's payload needs no items, resources or views, and the schema page should stay readable while the items directory is broken. It still answers 422 when the schema file itself does not load.
- **One envelope, three failure tiers.** Every endpoint answers `{ data, diagnostics, error }`; `diagnostics` is always present, often empty, so the client never optional-chains it. The tiers — project will not load (422), project loaded but this view will not render (200 with no data), success (200) — are identical across the view, schema, item, timer and git handlers. [ADR-013](adr/013-web-layer-contract.md) has the full contract; `crates/server/src/envelope.rs` is the code.
...
- **The browser learns about changes by ping, not by payload.** `crates/server/src/watcher.rs` debounces filesystem events into one broadcast ping per settled burst, and the SSE handler forwards it. Because there is no cache, the ping invalidates nothing; it only tells the browser to ask again. One channel per refresh scope: the generic file-change ping reloads the page's data, and the named `timer` and `git` events refetch only their own state — the git one fed by a second watcher on the repository's git directory (started only when `serve.git_controls` is on), so a terminal-side commit reaches the pill without a page's worth of refetching.
- **Git is the user's own `git`, in its own crate.** `crates/git` (`workdown-git`) is the one place git is spawned: it runs the `git` on `PATH` — credentials, hooks and signing behave as in a terminal — synchronously and under a timeout. Both the CLI (`workdown changes`, `workdown install-hooks`) and the server depend on it; neither depends on the other for git. The `/api/git` handlers (`crates/server/src/api/git.rs`) run each action on a blocking thread under one lock, so a pull and a push never overlap. Commit, pull-if-behind and push are one request with a per-step report; the dialog that confirms it is the review moment [ADR-014](adr/014-git-surface.md) requires.
- **The git controls touch only the workdown paths.** Staging, committing and the pill's dirty count are limited to a scope `workdown_git::scope` builds once per process from `Config::workdown_paths` — the `paths.*` entries, `schema`, and `config.yaml` itself, the same list the pre-commit hook installer reads — expressed as repository-relative pathspecs, so a workdown project inside a larger repository never touches its neighbours. Status still reads the whole repository once, because a pull must refuse over uncommitted work anywhere and name the files outside the scope.
- **The commit message comes from parsed frontmatter, not diff text.** `workdown_core::change_summary` compares each changed item's fields at `HEAD` and in the working tree, worded from the schema; a reordered field or a multi-line value cannot mislead it the way a line diff would. The preview that gathers those texts (`workdown_git::preview`) is also what `workdown changes` prints, so a terminal and the dialog word a change identically.
