...
- **Git is the user's own `git`, scoped to the workdown paths.** The `/api/git` handlers (`crates/server/src/api/git.rs`) shell out to the `git` on `PATH` — credentials, hooks and signing behave as in a terminal — under one lock, so a pull and a push never overlap. Every status, stage and commit is limited to a scope `crates/server/src/git_scope.rs` computes from the config: the `paths.*` entries, `schema`, and `config.yaml` itself, as repository-relative pathspecs, so a workdown project inside a larger repository never touches its neighbours. The commit message is generated by `crate::change_summary` in core from the parsed frontmatter of each changed item at `HEAD` and in the working tree — never from diff text. Commit, pull-if-behind and push are one request with a per-step report; the dialog that confirms it is the review moment ADR-006's "no auto-commit" rule asks for, and `workdown-items/full-git-loop.md` records the decision.
